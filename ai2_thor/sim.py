...
class AI2ThorSimulator:
    def __init__(self, scene, no_robot):
        # super(AI2ThorSimulator, self).__init__(test_log)

        # initialize simulator
        self.c = Controller(height=1000, width=1000)
        self.c.reset("FloorPlan" + str(scene))
        self.no_robot = no_robot  

        # initialize n agents into the scene
        self.multi_agent_event = self.c.step(dict(action='Initialize', agentMode="arm", snapGrid=False, gridSize=0.5, rotateStepDegrees=20, visibilityDistance=100, fieldOfView=90, agentCount=self.no_robot))
        ...
        # initialize n agents into the scene
        # self.multi_agent_event = self.c.step(dict(action='Initialize'))
